module Interpreter.State
    ...
    type state = { S: Map<string, int>; memory:memory}

    
    let mkState memsize = {S = Map.empty; memory = Memory.empty memsize} 
    ...
    let declare x st = 
        match st with 
        |{state.S =  map; state.memory=mem} when (not(Map.containsKey x map)) && (not(reservedVariableName x)) && validVariableName x -> 
            let newmap = Map.add x 0 map 
            Some({state.S = newmap; state.memory = mem})
    ...
    let setVar x v st =
        ...
        |{state.S = map; state.memory = mem} when (Map.containsKey x map) -> 
                                                                        ...
                                                                        Some({state.S = addv; state.memory = mem})
        |_ -> None          
    ...
    let alloc x size st =
        ...
        |{state.S = map; state.memory = mem} -> let newmemory = Memory.alloc size mem 
                                                ...
                                                match newstate with 
                                                |Some x -> Some({state.S = x.S; state.memory = mem;})
                                                |_ -> None  
    ...
    let free (ptr:int) (size:int) (st:state) =
        ...
        |{state.memory = mem} -> let freemem = Memory.free ptr size mem 
                                 ...
                                 match freemem with 
                                 |Some x -> Some{state.S = st.S; state.memory = x} 
                                 |None -> None
